...
def _build(cls, model, hass):
    """Construct a media player entity wired to a fake config entry."""
    entry = MagicMock(spec=ConfigEntry)
    entry.entry_id = "test"
    entry.data = {CONF_MODEL: model}
    player = cls(entry, "remote.ir_blaster")
    player.hass = hass
    player.entity_id = "media_player.cambridge_audio"
    return player


@pytest.fixture(autouse=True)
def no_write_state():
    """Stop async_write_ha_state from needing a fully registered entity."""
    with patch.object(CambridgeAudioCXA60MediaPlayer, "async_write_ha_state"):
        yield


@pytest.fixture
def mock_send():
    """Patch the infrared send so no real command goes out."""
    with patch(
        "custom_components.cambridge_audio_infrared.media_player."
        "InfraredEmitterConsumerEntity._send_command",
...
async def test_cxa60_turn_on(hass, mock_send, mock_rc5):
    ...
    player = _build(CambridgeAudioCXA60MediaPlayer, MODEL_CXA60, hass)

    await player.async_turn_on()

    mock_rc5.assert_called_once_with(address=RC5_SYSTEM_CODE, command=14)
    mock_send.assert_awaited_once_with(mock_rc5.sentinel)
...
async def test_cxa80_bluetooth_source(hass, mock_send, mock_rc5):
    ...
    player = _build(CambridgeAudioCXA80MediaPlayer, MODEL_CXA80, hass)
...
def test_cxa80_has_bluetooth_in_source_list():
